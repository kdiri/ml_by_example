...
regularizator_theta1 = (lambda / (2 * m)) * sum(sum(Theta1(:,2:end).^2));
regularizator_theta2 = (lambda / (2 * m)) * sum(sum(Theta2(:,2:end).^2));
delta_error = regularizator_theta1 + regularizator_theta2;
...
%% Rollback to calculate the residuals
delta3= a3 - yhat;
